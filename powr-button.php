<?php
    ...
    Plugin URI: https://www.powr.io/plugins/button
    ...
    */
    ...
    class Powr_Button extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Button.
        </p>
        <p>
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_button_menu' );
    function powr_button_menu() {
      add_menu_page( 'POWr Button', 'POWr Button', 'manage_options', 'powr-button-settings', 'powr_button_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_button_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/button';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/button';
      }
    ...
    register_activation_hook(__FILE__, 'powr_button_plugin_activate');
      add_action('admin_init', 'powr_button_plugin_redirect');

      function powr_button_plugin_activate() {
      add_option('powr_button_plugin_do_activation_redirect', true);
      }
      ...
      function powr_button_plugin_redirect() {
      if (get_option('powr_button_plugin_do_activation_redirect', false)) {
          delete_option('powr_button_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-button-settings&' );
          }
       }
      }
